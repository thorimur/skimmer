...
def Lean.Language.Lean.CommandParsedSnapshot.EIO.runCommandElabM (ictx : Parser.InputContext)
    (x : CommandElabM α) (snap : CommandParsedSnapshot) : EIO Exception (α × State) := do
  x.run (snap.toCommandCtx ictx) |>.run (snap.elabSnap.resultSnap.get.cmdState)

def Lean.Language.Lean.CommandParsedSnapshot.EIO.runCommandElabM' (ictx : Parser.InputContext)
    (x : CommandElabM α) (snap : CommandParsedSnapshot) : EIO Exception α := do
  x.run (snap.toCommandCtx ictx) |>.run' (snap.elabSnap.resultSnap.get.cmdState)

def Lean.Language.Lean.CommandParsedSnapshot.runCommandElabM (ictx : Parser.InputContext)
    (x : CommandElabM α) (snap : CommandParsedSnapshot) : IO (Except Exception (α × State)) := do
  x.run (snap.toCommandCtx ictx) |>.run (snap.elabSnap.resultSnap.get.cmdState) |>.toIO'

def Lean.Language.Lean.CommandParsedSnapshot.runCommandElabM' (ictx : Parser.InputContext)
    (x : CommandElabM α) (snap : CommandParsedSnapshot) : IO (Except Exception α) := do
  x.run (snap.toCommandCtx ictx) |>.run' (snap.elabSnap.resultSnap.get.cmdState) |>.toIO'
...
public def main : IO Unit := do
  ...
  let mut edits : Array String := #[] -- import actual `Edit` functionality here
  for snap in commands do
    -- TODO: could be useful to run in new state, but this is really a subprocess-version problem.
    match ← snap.runCommandElabM' inputCtx <| replaceDeprecated replacements edits with
