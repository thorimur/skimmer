...
public import Skimmer.Lean.Data.Array
public import Skimmer.LinterWithCleanup.PunchCard

@[expose] public section

abbrev RangeBoundariesMod2 := Std.HashSet String.Pos.Raw

namespace RangeBoundariesMod2

open Lean

def insertMod2 (m : RangeBoundariesMod2) (k : String.Pos.Raw) : RangeBoundariesMod2 :=
  if m.contains k then m.erase k else m.insert k

def insertRange (m : RangeBoundariesMod2) (r : Syntax.Range) : RangeBoundariesMod2 :=
  m.insertMod2 r.start |>.insertMod2 r.stop

def isRange (m : RangeBoundariesMod2) (start stop : String.Pos.Raw) : Bool :=
...
def IO.recordRange (idx : Nat) (range : Syntax.Range) : BaseIO Unit := do
